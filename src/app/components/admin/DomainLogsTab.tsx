...
interface DomainLogsTabProps {
  domainId: number;
  domainName: string;
}

export const DomainLogsTab: React.FC<DomainLogsTabProps> = ({ domainId, domainName }) => {
  ...
  const loadLogs = async (page: number) => {
    setLoading(true);
    try {
      const data = await auditLogsService.list({ page, domain_id: domainId });
      setLogs(data.logs || []);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Logi domeny {domainName}</CardTitle>
        <CardDescription>Historia operacji związanych z domeną</CardDescription>
      </CardHeader>
